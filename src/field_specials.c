...
{
    if (FlagGet(FLAG_ENABLE_BETTI_FIRST_CALL))
    {
        switch (gMapHeader.mapType)
        {
            case MAP_TYPE_TOWN:
            ...
            case MAP_TYPE_ROUTE:
            case MAP_TYPE_OCEAN_ROUTE:
                if (++(*GetVarPointer(VAR_BETTI_CALL_STEP_COUNTER)) < 250)
                {
                    return FALSE;
                }
                break;
        ...
        graphicsId == OBJ_EVENT_GFX_RIVAL_MAY_NORMAL)
    {
        ...
        if (obj != OBJECT_EVENTS_COUNT)
        {
            ...
            case OBJ_EVENT_GFX_LINK_RS_AKIRA:
                LoadPalette(gObjectEventPal_RubySapphireAkira, 0x100 + (adjustedPaletteNum << 4), 0x20);
                break;
            ...
            case OBJ_EVENT_GFX_RIVAL_AKIRA_NORMAL:
                LoadPalette(gObjectEventPal_Akira, 0x100 + (adjustedPaletteNum << 4), 0x20);
                break;
...
static void PCTurnOnEffect_1(s16 isPcTurnedOn, s8 dx, s8 dy)
{
    ...
    if (isPcTurnedOn)
    {
        ...
        {
            tileId = METATILE_AkirasMaysHouse_AkiraPC_Off;
        }
        else if (gSpecialVar_0x8004 == PC_LOCATION_MAYS_HOUSE)
        {
            tileId = METATILE_AkirasMaysHouse_MayPC_Off;
        }
    }
    else
    {
        ...
        {
            tileId = METATILE_AkirasMaysHouse_AkiraPC_On;
        }
        else if (gSpecialVar_0x8004 == PC_LOCATION_MAYS_HOUSE)
        {
            tileId = METATILE_AkirasMaysHouse_MayPC_On;
        }
    }
