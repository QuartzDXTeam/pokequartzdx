        ...
        for (i = 0; i < gNumLinkContestPlayers; i++)
        {
            ...
            if (version == VERSION_RUBY || version == VERSION_SAPPHIRE)
            {
                if (gLinkPlayers[i].gender == MALE)
                    gContestMons[i].trainerGfxId = OBJ_EVENT_GFX_LINK_RS_AKIRA;
                else
...
void LoadLinkContestPlayerPalettes(void)
{
    ...
    if (gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK)
    {
        for (i = 0; i < gNumLinkContestPlayers; i++)
        {
            ...
            {
                if (gLinkPlayers[i].gender == MALE)
                    LoadPalette(gObjectEventPal_RubySapphireAkira, 0x160 + i * 0x10, 0x20);
                else
            ...
            {
                if (gLinkPlayers[i].gender == MALE)
                    LoadPalette(gObjectEventPal_Akira, 0x160 + i * 0x10, 0x20);
                else
