...
static void Task_NewGameBirchSpeech_SlideOutOldGenderSprite(u8 taskId)
{
    ...
    else
    {
        ...
        if (gTasks[taskId].tPlayerGender != MALE)
            spriteId = gTasks[taskId].tRanSpriteId;
        else
...
static void Task_NewGameBirchSpeech_ReshowBirchLotad(u8 taskId)
{
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[gTasks[taskId].tRanSpriteId].invisible = TRUE;
        spriteId = gTasks[taskId].tBirchSpriteId;
        gSprites[spriteId].pos1.x = 136;
        gSprites[spriteId].pos1.y = 60;
        gSprites[spriteId].invisible = FALSE;
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    ranSpriteId = CreateTrainerSprite(FacilityClassToPicIndex(FACILITY_CLASS_RAN), 120, 60, 0, &gDecompressionBuffer[0x800]);
    gSprites[ranSpriteId].callback = SpriteCB_Null;
    gSprites[ranSpriteId].invisible = TRUE;
    gSprites[ranSpriteId].oam.priority = 0;
    gTasks[taskId].tRanSpriteId = ranSpriteId;
}
