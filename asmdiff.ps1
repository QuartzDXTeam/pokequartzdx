...
if (-Not (Test-Path ".\pokequartzdx.gba"))
{
    Write-Host "File 'pokequartzdx.gba' not found."
    Write-Host $help
    exit
}
...
&$objdump -D -bbinary -marmv4t -Mforce-thumb --start-address="$start_str" --stop-address="$end_str" .\pokequartzdx.gba > .\pokequartzdx.dump
Invoke-Expression "$DiffTool .\baserom.dump .\pokequartzdx.dump"
